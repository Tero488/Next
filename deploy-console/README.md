...
3. 在对应板块卡片里操作：
   - **案例**：拖入一组案例图（第一张自动作封面）、填标题/分类/描述 → 点「发布案例」
   - **新闻**：拖入一张封面图、填标题/日期/摘要 → 点「发布新闻」
   - **加入我们**：填职位名 + 多条任职要求（每行一条）→ 点「发布职位」
4. 控制台自动完成：
   - 图片转成与全站一致的 **webp**（sharp，质量 85，不损分辨率）；
   - 按规律命名落盘（案例 `public/images/cases/<id>/`，新闻 `public/images/news/<id>.webp`）；
   - 往 `data/index.ts` 的中/英数组里插入新条目（案例/新闻插到最前，招聘插到最后）；
   - 通过 **GitHub REST API 直接提交**到 `Tero488/Next` 的 main → **Vercel 自动部署**。
5. 几秒后线上对应页面就出现新内容（案例 `#/cases`、新闻 `#/news`、加入我们 `#/join`）。
...
## 接口
- `POST /api/add-content` —— 拖拽发布（body：`{ section: "cases"|"news"|"jobs", ...字段 }`）
